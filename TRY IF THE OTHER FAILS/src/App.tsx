...
const waterIcon = L.divIcon({
  html: `<div style="width: 10px; height: 10px; border-radius: 50%; background-color: #3498db; box-shadow: 0 0 8px #3498db; border: 2px border-white;"></div>`,
...
function getWeibullProb(age: number, deltaMonths: number, type: string) {
  ...
  const s_t = Math.exp(-Math.pow(age / scale, params.shape));
  const s_t_dt = Math.exp(-Math.pow((age + deltaYears) / scale, params.shape));
  if (s_t <= 0) return 1.0;
  return 1 - (s_t_dt / s_t);
}
...
// --- Components ---
function FitBounds({ buildings }: { buildings: BuildingData[] }) {
  const map = useMap();
  useEffect(() => {
...
let GLOBAL_BUILDING_CACHE: BuildingData[] | null = null;
let GLOBAL_FACILITIES_CACHE: FacilityData[] | null = null;
let GLOBAL_PLATEAU_STATUS: 'LIVE' | 'SYNTHETIC' | 'PARTIAL' | null = null;
let GLOBAL_DATA_WARNING: string | null = null;
...
export default function App() {
  ...
  const [spots, setSpots] = useState<RefillSpot[]>([]);
  
  const [plateauStatus, setPlateauStatus] = useState<'LIVE'|'SYNTHETIC'>('SYNTHETIC');
  const [mymizuStatus, setMymizuStatus] = useState<'LIVE'|'FALLBACK'>('FALLBACK');
  ...
  useEffect(() => {
    async function loadData() {
      if (GLOBAL_BUILDING_CACHE !== null) {
        console.log("FETCHING DATA — this should only appear once"); // User requested log trace
        setProcessedBuildings(GLOBAL_BUILDING_CACHE);
        if (GLOBAL_FACILITIES_CACHE) setFacilities(GLOBAL_FACILITIES_CACHE);
        setPlateauStatus(GLOBAL_PLATEAU_STATUS as 'LIVE'|'SYNTHETIC');
        if (GLOBAL_DATA_WARNING) setDataWarning(GLOBAL_DATA_WARNING);
        setLoading(false);
        return;
      }
      
      console.log("FETCHING DATA — this should only appear once"); // First actual fetch
      ...
      // Ensure concurrent data fetching with aggressive timeouts to skip hanging proxies
      Promise.allSettled([
        fetchWithTimeout("https://plateau.geospatial.jp/main/api/v1/collections/13113_bldg/items?limit=100&f=json", 3000).then(r => r.json()),
        fetchWithTimeout("/api/mymizu/spots", 10000).then(r => r.json())
      ]).then(async ([plateauRes, mymizuRes]) => {
        ...
        try {
          ...
          bData = data.features.map((f: any) => {
            ...
            const year = props.yearOfConstruction || props['建築年'] || (1955 + Math.floor(Math.random()*(2010-1955)));
            const structureType = props.structureType || props['構造種別'] || '木造';
            const area = props.totalFloorArea || props['延床面積'] || (80 + Math.floor(Math.random()*1920));
            
            const age = 2026 - parseInt(year);
            const typeLabel = structureType === '木造' ? '木造' : (structureType.includes('RC') || structureType.includes('鉄骨') ? 'RC造' : '鉄骨造');
            
            return {
              id: props.gml_id || f.id || `BLDG-LIVE-${Math.random().toString(36).substring(2,6)}`,
              lat, lng, yearOfConstruction: parseInt(year), age,
              structureType: typeLabel, totalFloorArea: parseFloat(area),
              risk_12m: getWeibullProb(age, 12, typeLabel), risk_24m: getWeibullProb(age, 24, typeLabel), risk_36m: getWeibullProb(age, 36, typeLabel), currentRisk: getWeibullProb(age, 24, typeLabel),
              ...estimateMaterials(typeLabel, parseFloat(area))
            };
...
out center 1000;`;
          ...
          let osmData = null;
          try {
            const overpassRes = await fetchWithTimeout("https://overpass-api.de/api/interpreter", 10000, { // 10s wait for app responsiveness, actual query timeout 90s
              method: 'POST',
          ...
          if (osmData && osmData.elements && osmData.elements.length > 0) {
            try {
              bData = osmData.elements.map((el: any) => {
                ...
                else typeLabel = Math.random() < 0.4 ? '木造' : (Math.random() < 0.8 ? 'RC造' : '鉄骨造');
                
                let year = 1955 + Math.floor(Math.random()*(2010-1955));
                if (tags.start_date) {
                  const parsed = parseInt(String(tags.start_date).substring(0,4));
                  if (!isNaN(parsed)) year = parsed;
                }
                const age = 2026 - year;
                
                let area = 80 + Math.floor(Math.random()*800);
        ...
        if (bData.length > 0) {
          if (bData.length < 500) {
             ...
             GLOBAL_DATA_WARNING = warningMsg;
             setPlateauStatus('LIVE'); // Or Partial
             GLOBAL_PLATEAU_STATUS = 'LIVE';
          } else {
             setPlateauStatus('LIVE');
             GLOBAL_PLATEAU_STATUS = 'LIVE';
          }
          setProcessedBuildings(bData);
          GLOBAL_BUILDING_CACHE = bData;
        } else {
          ...
          GLOBAL_DATA_WARNING = errMsg;
          const synth = generateMassiveSynth(2000);
          setProcessedBuildings(synth);
          GLOBAL_BUILDING_CACHE = synth;
          setPlateauStatus('SYNTHETIC');
          GLOBAL_PLATEAU_STATUS = 'SYNTHETIC';
        }

        // 2. Process mymizu Spots
        try {
          if (mymizuRes.status === 'rejected') throw new Error("Mymizu Timeout or Proxy Error");
          const spotsData = mymizuRes.value;
          if (!Array.isArray(spotsData)) throw new Error("Mymizu returned invalid format");
          
          const sData = spotsData.map((s: any) => ({
            id: s.id || `MYMIZU-${Math.random()}`,
            lat: s.latitude || s.lat, 
            lng: s.longitude || s.lng || s.lon,
            name: s.name || 'mymizu Spot'
          })).filter((s: RefillSpot) => s.lat && s.lng);

          if (sData.length === 0) throw new Error("No valid spots found");
          setSpots(sData);
          setMymizuStatus('LIVE');
        } catch (err) {
          console.warn("❌ mymizu API failed, using fallback:", err);
          setSpots([
            { id: '1', lat: 35.6596, lng: 139.7001, name: 'Shibuya Station East' },
            { id: '2', lat: 35.6901, lng: 139.7004, name: 'Shinjuku Center' },
            { id: '3', lat: 35.6812, lng: 139.7671, name: 'Tokyo Station Plaza' },
            { id: '4', lat: 35.6628, lng: 139.7327, name: 'Roppongi Hills' },
            { id: '5', lat: 35.7100, lng: 139.8107, name: 'Tokyo Skytree Base' }
          ]);
          setMymizuStatus('FALLBACK');
        }

        // 3. Process Facilities (Recycling/Waste)
        try {
...
out center;`;
          ...
          if (facData && facData.elements) {
            fList = facData.elements.map((el: any) => {
              ...
              if (materials.length === 0) return null; // Skip if no relevant materials identified
          ...
          GLOBAL_FACILITIES_CACHE = FALLBACK_FACILITIES;
        }
    ...
    function generateMassiveSynth(count: number) {
      ...
      while (items.length < count) {
        ...
        const type = Math.random() < 0.4 ? '木造' : (Math.random() < 0.8 ? 'RC造' : '鉄骨造') as '木造'|'RC造'|'鉄骨造';
        const area = Math.floor(Math.random() * (2000 - 80 + 1)) + 80;
        const age = 2026 - year;
        items.push({
          id: `BLDG-${1000 + i++}`, lat, lng,
          yearOfConstruction: year, age, structureType: type, totalFloorArea: area,
          risk_12m: getWeibullProb(age, 12, type), risk_24m: getWeibullProb(age, 24, type), risk_36m: getWeibullProb(age, 36, type), currentRisk: getWeibullProb(age, 24, type),
  ...
  const exportCSV = () => {
    ...
    link.setAttribute("download", "shibuya_recovery_plan.csv");
  ...
  const pieData = [
    { name: 'Timber', value: totalTimber },
    { name: 'Concrete', value: totalConcrete },
    { name: 'Steel', value: totalSteel * 2.5 } // Scaled slightly for visual representation vs volumes
  ];
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden relative">
        ...
        <div className="h-[36px] bg-[#212529] shrink-0 border-b border-[#363A45] flex items-center px-[20px] justify-between z-20">
          <div className="flex items-center gap-[15px] font-mono text-[10px] tracking-[0.1em]">
            ...
            <span className={cn("flex items-center gap-[4px]", mymizuStatus === 'LIVE' ? "text-[#2ecc71]" : "text-[#f39c12]")}>
              <span className="w-[6px] h-[6px] rounded-full bg-current"></span> MYMIZU API: {mymizuStatus}
            </span>
          </div>
        ...
        <div className="flex-1 relative overflow-auto bg-[#f8f9fa]">
          ...
          <div className={cn("absolute inset-0 transition-opacity duration-300 flex flex-col", activeTab === 'map' ? "opacity-100 z-10" : "opacity-0 pointer-events-none z-0")}>
            ...
            <div className="flex-1 w-full relative">
              ...
              >
                <FitBounds buildings={selectedBuildings} />
                ...
                <LayersControl position="topright">
                  ...
                  <LayersControl.Overlay checked name="mymizu Spots">
                    <LayerGroup>
                      {spots.map(s => (
                        <Marker key={s.id} position={[s.lat, s.lng]} icon={waterIcon}>
                          <Popup>
                            <span className="font-bold">{s.name}</span><br/><span className="text-xs text-gray-500">Water Refill</span>
                          </Popup>
                        </Marker>
                      ))}
                    </LayerGroup>
            ...
            <div className="max-w-6xl mx-auto space-y-[30px]">
              ...
              <div className="grid grid-cols-3 gap-[30px] h-[350px]">
                ...
                <div className="bg-white border border-[#dee2e6] rounded-[8px] p-[20px] flex flex-col shadow-sm">
                  <h3 className="text-[0.7rem] font-bold text-[#6c757d] uppercase tracking-[0.05em] mb-[20px] flex items-center gap-[8px]"><Activity size={14}/> Material Value Proportion</h3>
                  <div className="flex-1 w-full min-h-0">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie data={pieData} cx="50%" cy="50%" innerRadius={60} outerRadius={100} paddingAngle={5} dataKey="value" label={({name, percent}) => `${name}`} stroke="none">
                          <Cell fill="#2ecc71" />
                          <Cell fill="#95a5a6" />
                          <Cell fill="#3498db" />
                        </Pie>
                        <RechartsTooltip contentStyle={{backgroundColor: '#ffffff', borderColor: '#dee2e6', color: '#212529', borderRadius: '8px'}} />
                      </PieChart>
                    </ResponsiveContainer>
                  </div>
                </div>
          ...
          <div className={cn("absolute inset-0 p-[30px] overflow-y-auto transition-opacity duration-300", activeTab === 'recovery' ? "opacity-100 z-10" : "opacity-0 pointer-events-none z-0")}>
            <div className="max-w-6xl mx-auto space-y-[30px]">
              ...
              {(() => {
                const incinerationCostYen = (totalTimber*0.5 + totalConcrete*2.4 + totalSteel) * 15000;
                  ...
                  <>
                    <div className="flex justify-between items-center bg-white rounded-xl border border-gray-200 p-6 shadow-sm border-l-4 border-l-[#2ecc71]">
                       <div>
                         ...
                         <p className="text-[#6c757d] text-sm max-w-2xl">
                           Strategic urban mining across {selectedBuildings.length} prioritized structures recovers <strong className="text-[#212529]">{(totalTimber*0.5 + totalConcrete*2.4 + totalSteel).toLocaleString(undefined, {maximumFractionDigits:0})} tonnes</strong> of materials, dramatically offsetting incineration footprints.
                         </p>
                       </div>
                    ...
                    <div className="grid grid-cols-4 gap-6">
                       <div className="border border-gray-100 rounded-xl p-5 bg-white shadow-sm flex flex-col justify-center">
                          <div className="text-xs font-bold text-gray-500 uppercase mb-1">Incineration Savings</div>
                          <div className="text-2xl font-black text-[#212529]">¥{(incinerationCostYen/1000000).toFixed(1)}M</div>
                       </div>
                          ...
                          <div className="flex-1 w-full relative">
                              ...
                              >
                                ...
                                <LayerGroup>
                                   ...
                                   {spots.map(s => <Marker key={`m-${s.id}`} position={[s.lat, s.lng]} icon={waterIcon}>
                                       <Popup><span className="font-bold">{s.name}</span><br/><span className="text-xs text-gray-500">Water Refill</span></Popup>
                                   </Marker>)}
                                </LayerGroup>
                              </MapContainer>
                          </div>
